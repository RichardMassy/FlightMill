...
# Time taken to initiate a flight
df1$Flight_initiation = with(df1,(14400-Flight_time) / N_flights)
...
summary(m10)
# Final model. D.O: Size-2.93, Direction-2.00, Morph:Sex-1.46, Sex-0.95
...
m10f = lm(log(Flight_initiation) ~ Morph + Condition + Morph:Condition,
          data=df1[df1$Sex=="Female",])
summary(m10f)
...
ggplot(df1,aes(x=Size,y=Mean_speed,fill="Black"))+
  geom_boxplot(notch=F,outlier.shape=NA,alpha=0.05)+
  geom_point(position=position_jitterdodge(),size=1)+
  scale_x_discrete(labels=c("Small","Moderate","Large"))+
  theme_classic()+
...
# N_flights
ggplot(df1,aes(x=Condition,y=N_flights,color=Morph,fill=Morph))+
  scale_y_log10()+
  geom_boxplot(notch=F,outlier.shape=NA,alpha=0.1)+
  geom_point(position=position_jitterdodge(),size=1)+
  scale_color_manual(values=cbPalette[c(6,7)],name="Morph:")+
...
# Flight initiation
ggplot(data=df1,aes(x=Condition,y=Flight_initiation,color=Morph,fill=Morph))+
  scale_y_log10()+
  geom_boxplot(notch=F,outlier.shape=NA,alpha=0.1)+
  geom_point(position=position_jitterdodge(),size=1)+
  scale_color_manual(values=cbPalette[c(6,7)],name="Morph:")+
  ...
  theme(legend.position="top",legend.justification="right",text=element_text(size=15),
        panel.grid.minor=element_blank(),panel.grid.major.x=element_blank())+
  labs(y="Mean time taken to initiate flight (s)",x="\nCondition")
